...
import Kairos.PfPat (PfPat (pat, pfId))
import Kairos.Pfield (PfId, PfMap, Pfield (Pd, Ps), newPfId, pfIdInt)
...
data Instr = I
  { -- | Instrument ID : Int
    insN :: InstrumentID,
    -- | Pfields and their id
    pf :: TVar PfMap,
    -- | is the instrument playing ?
    status :: Status,
    -- | when to play the instrument next
    toPlay :: Maybe TimePoint,
    -- | Patterns of Parameters and their IDs
    pats :: TVar (M.Map Int PfPat),
    -- | Name of the time function to refer to
    timeF :: String,
    -- | where to send the message (Csound or OSC)
    kind :: MessageTo UDPPort,
    ...
    newTVarIO $
      pfFromList
        [ (newPfId 3 "dur", Pd 1),
          ...
          (newPfId 6 "del", Pd 0),
          (newPfId 7 "pan", Pd 64),
          (newPfId 8 "chan", Pd chan),
          (newPfId 9 "pitch", Pd 60),
          (newPfId 10 "vel", Pd 115),
          (pfIdInt 11, Pd 0),
          (pfIdInt 12, Pd 20),
          (pfIdInt 13, Pd 50),
          (pfIdInt 14, Pd 50),
